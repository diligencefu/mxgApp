...
- (void)loadAdditionalUI
{
    ...
    NSTimeInterval actionTimeout = 10;
...
- (void)requestAuth
{
    ...
    [_wrapView startAuthWithCompletionBlk:^(NSError * _Nonnull error) {
        ...
        if (error) {
            if ((strongSelf != nil) && (strongSelf.detectionFailedBlk != nil)) {
                strongSelf.detectionFailedBlk(strongSelf, errorInfo);
            } else {
...
- (void)onDetectionFailed:(AAIDetectionResult)detectionResult forDetectionType:(AAIDetectionType)detectionType
{
    ...
    if (key) {
        ...
        if (self.detectionFailedBlk) {
            self.detectionFailedBlk(self, @{@"key": key, @"state": state});
        } else {
